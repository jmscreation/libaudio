...
class SoundBuffer {
	short* samples;
	size_t sampleCount, bufferPos;
	int sampleRate, channels, sampleBytes;
	float sampleFactor;
	float defVolume;
	bool defDestroy;
...
public:
	...
	bool preallocateSamples(size_t len, int rate, int channs, int bytes);
	bool appendSamples(const void* data, size_t size);
